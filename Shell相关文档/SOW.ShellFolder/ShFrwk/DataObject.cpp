...
CLIPFORMAT CDataObject::s_cfSHELLIDLIST = (CLIPFORMAT)::RegisterClipboardFormat(CFSTR_SHELLIDLIST);
CLIPFORMAT CDataObject::s_cfDROPEFFECT = (CLIPFORMAT)::RegisterClipboardFormat(CFSTR_PREFERREDDROPEFFECT);
...
STDMETHODIMP CDataObject::GetData(FORMATETC* pFormatetc, STGMEDIUM* pMedium)
{
   ...
   //if( pFormatetc->cfFormat == s_cfDROPEFFECT )
   //{
   //   DWORD dwValue = DROPEFFECT_COPY|DROPEFFECT_LINK;
   //   return _SetHGLOBAL(pMedium, &dwValue, sizeof(dwValue));
   //}

   if( pFormatetc->cfFormat == s_cfSHELLIDLIST && _ShellModule.GetConfigBool(VFS_HAVE_VIRTUAL_FILES) ) 
   {
	  //// reference link: https://github.com/aarondunn/simpleshare/blob/master/windows/boxshellext/dataobject.cpp
   //   if( !IsBitSet(pFormatetc->tymed, TYMED_HGLOBAL) ) return DV_E_TYMED;

	  //UINT uItemCount = m_pidls.GetItemCount();
	  //CIDA cida = { uItemCount, 0 };
	  //UINT cbPidl;
	  //// get the size of the parent folder's PIDL
	  //cbPidl = m_spFolder->m_pidlMonitor.GetByteSize();

	  //UINT i;
	  //// get the total size of all of the PIDLs
	  //for (i=0; i<m_pidls.GetItemCount(); i++)
	  //{
		 // UINT uByte = ::ILGetSize(m_pidls.GetItem(i));
		 // cbPidl += uByte;
	  //}
	  ...
	  //DWORD dwTotalSize = iCurPos + // size of the CIDA structure and the additional aoffset elements
			//			cbPidl + 1; // size of the pidls
   //   CAutoVectorPtr<BYTE> buffer( new BYTE[dwTotalSize] );
	  //LPIDA pData = (LPIDA)buffer;
	  //pData->cidl = uItemCount;
	  //pData->aoffset[0] = iCurPos;
		
	  //cbPidl = m_spFolder->m_pidlMonitor.GetByteSize();;
	  //CopyMemory((LPBYTE)(pData) + iCurPos, (LPBYTE)m_spFolder->m_pidlMonitor, cbPidl);
	  //iCurPos += cbPidl;
	  //for (i=0; i<uItemCount; i++)
	  //{
		 // // get the size of the PIDL
		 // cbPidl = ::ILGetSize(m_pidls.GetItem(i));

		 // // fill out the members of the CIDA structure.
		 // pData->aoffset[i + 1] = iCurPos;

		 // // copy the contents of the PIDL
		 // CopyMemory((LPBYTE)(pData) + iCurPos, (LPBYTE)m_pidls.GetItem(i), cbPidl);
