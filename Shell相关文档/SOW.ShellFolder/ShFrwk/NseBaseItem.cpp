...
HRESULT CNseBaseItem::GetChildDirect(LPCWSTR pstrFullName, CNseItem** pItem)
{
   return E_NOTIMPL;
}
...
HRESULT CNseBaseItem::_DoPasteFiles(VFS_MENUCOMMAND& Cmd)
{
   ...
   //{
   //   // create an operation without delete confirmation(during moving, there should be no delete confirmation)
	  //Cmd.pFO->SetOperationFlags(FOF_NOCOPYSECURITYATTRIBS | FOF_NOCONFIRMMKDIR | FOFX_NOSKIPJUNCTIONS | FOF_NOCONFIRMATION);
	  //Cmd.pFO->DeleteItems(Cmd.pDataObject);
   //}
   if( Cmd.dwDropEffect == DROPEFFECT_MOVE )
	   Cmd.pFO->MoveItems(Cmd.pDataObject, spTargetFolder);
   else
	   Cmd.pFO->CopyItems(Cmd.pDataObject, spTargetFolder);
...
HRESULT CNseBaseItem::_DoNewFolder(VFS_MENUCOMMAND& Cmd, UINT uLabelRes)
{
   ...
   IFileOperationProgressSink* pSink = NULL;
   CComObject<CFileOpProgSink>* pFileOpSink = NULL;
   HR( CComObject<CFileOpProgSink>::CreateInstance(&pFileOpSink) );
   pFileOpSink->QueryInterface(IID_PPV_ARGS(&pSink));
   spFO->NewItem(spTargetFolder, FILE_ATTRIBUTE_DIRECTORY, bstrLabel, NULL, pSink);
   HR( spFO->PerformOperations() );
   LPCWSTR pName = pFileOpSink->GetResultName();
   if (pName)
   {
	   bstrLabel = pName;
   }
...
////////////////////////////////////////////////////////////////////////////////////////
// CFileOpProgSink
CFileOpProgSink::CFileOpProgSink() : _hwnd(NULL)
{
	memset(_szItemName, 0, sizeof(_szItemName));
}

void CFileOpProgSink::FinalRelease()
{
	ATLTRACE(L"CFileOpProgSink::FinalRelease\n");
}

// IFileOperationProgressSink
IFACEMETHODIMP CFileOpProgSink::StartOperations()
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::FinishOperations(HRESULT hrResult)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PreRenameItem(DWORD /*dwFlags*/, IShellItem * /*psiItem*/, PCWSTR /*pszNewName*/)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PostRenameItem(DWORD dwFlags, IShellItem * psiItem, PCWSTR pszNewName, HRESULT hrRename, IShellItem * psiNewlyCreated)
{
	if (SUCCEEDED(hrRename))
	{
		memset(_szItemName, 0, sizeof(_szItemName));
		wcscpy_s(_szItemName, lengthof(_szItemName)-1, pszNewName);
	}
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PreMoveItem(DWORD /*dwFlags*/, IShellItem * /*psiItem*/, IShellItem * /*psiDestinationFolder*/, PCWSTR /*pszNewName*/)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PostMoveItem(DWORD dwFlags, IShellItem * psiItem,
    IShellItem * psiDestinationFolder, PCWSTR pszNewName, HRESULT hrNewName, IShellItem * psiNewlyCreated)
{
	if (SUCCEEDED(hrNewName))
	{
		memset(_szItemName, 0, sizeof(_szItemName));
		wcscpy_s(_szItemName, lengthof(_szItemName)-1, pszNewName);
	}
...
IFACEMETHODIMP CFileOpProgSink::PreCopyItem(DWORD dwFlags, IShellItem *psiItem,
    IShellItem *psiDestinationFolder, PCWSTR pszNewName)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PostCopyItem(DWORD dwFlags, IShellItem *psiItem,
    ...
    IShellItem *psiNewlyCreated)
{
	...
	{
		memset(_szItemName, 0, sizeof(_szItemName));
		wcscpy_s(_szItemName, lengthof(_szItemName)-1, pwszNewName);
	}
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PreDeleteItem(DWORD /*dwFlags*/, IShellItem * /*psiItem*/)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PostDeleteItem(DWORD /*dwFlags*/, IShellItem * /*psiItem*/, HRESULT /*hrDelete*/, IShellItem * /*psiNewlyCreated*/)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PreNewItem(DWORD /*dwFlags*/, IShellItem * /*psiDestinationFolder*/, PCWSTR /*pszNewName*/)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PostNewItem(DWORD dwFlags, IShellItem * psiDestinationFolder,
...
IFACEMETHODIMP CFileOpProgSink::UpdateProgress(UINT iWorkTotal, UINT iWorkSoFar)
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::ResetTimer()
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::PauseTimer()
{
    return S_OK;
}
IFACEMETHODIMP CFileOpProgSink::ResumeTimer()
{
    return S_OK;
}

// return the actual item name after operation(e.g, when create a new item, the name could be different to the original if duplicated)
LPCWSTR CFileOpProgSink::GetResultName()
{
	return _szItemName;
}
