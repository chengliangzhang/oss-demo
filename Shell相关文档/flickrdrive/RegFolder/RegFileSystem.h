...
public:
   ...
   HRESULT _OpenRegKey(REGOPEN_ACC Type, UINT uAccess, REGOPEN_PATH PathPart, LPCTSTR pszSubKey, CRegKey& key) const;
   HRESULT _GetRegPathQuick(PCIDLIST_RELATIVE pidlPath, PCITEMID_CHILD pidlChild, HKEY& hKeyRoot, LPWSTR pszPath) const;
...
public:
   ...
   HRESULT _OpenRegKey(UINT uAccess, CRegKey& key) const;
   HRESULT _GetRegValue(CComPropVariant& v) const;
   HRESULT _SetRegValue(const CComPropVariant& v) const;
   HRESULT _GetPathnameQuick(PCIDLIST_RELATIVE pidlPath, PCITEMID_CHILD pidlChild, BOOL bParsingPath, HKEY& hKeyRoot, LPWSTR pszPath) const;
